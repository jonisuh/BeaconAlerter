...
class BeaconInfo : NSObject {
    ...
    public init(beaconID: BeaconID, txPower: Int, RSSI: Int, telemetry: NSData?, deviceName: String) {
        self.beaconID = beaconID
    ...
    ///
    class func beaconInfoForUIDFrameData(frameData: NSData, telemetry: NSData?, RSSI: Int, deviceName: String)
        -> BeaconInfo? {
            if frameData.length > 1 {
                let count = frameData.length
                var frameBytes = [UInt8](repeating: 0, count: count)
                ...
                return BeaconInfo(beaconID: bid, txPower: txPower, RSSI: RSSI, telemetry: telemetry, deviceName: deviceName)
            }
            
            return nil
    }
    
    class func beaconInfoForEIDFrameData(frameData: NSData, telemetry: NSData?, RSSI: Int, deviceName: String)
        -> BeaconInfo? {
            if frameData.length > 1 {
                let count = frameData.length
                var frameBytes = [UInt8](repeating: 0, count: count)
    ...
    class func hexaDecimalString(from: [UInt8]) -> String{
        var hexString = ""
        
        for int in from{
            hexString = hexString + String(int, radix: 16, uppercase: false)
        }
